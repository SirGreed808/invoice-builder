              ...
              <tbody>
                {quotes.map((q) => (
                  <tr key={q.id} onClick={() => (window.location.href = `/quotes/${q.id}`)} style={{ cursor: 'pointer' }}>
                    <td style={{ fontFamily: 'var(--mono)', fontSize: '0.82rem' }}>{q.number}</td>
                    <td>{clientMap[q.clientId]?.name ?? '—'}</td>
                    <td style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{formatDate(q.createdAt)}</td>
                    <td style={{ fontFamily: 'var(--mono)' }}>{formatCurrency(total(q.lineItems, q.taxRate))}</td>
